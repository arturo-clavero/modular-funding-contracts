...
contract CrowdFunding is FundBase, AutomationCompatibleInterface {
    ...
    modifier whiteListAllowance() {
        if (isWhiteListed && whiteList[msg.sender] == false) revert NotAuthorized();
        _;
    }
    ...
    constructor(
        ...
        target = _target;
        deadline = _seconds * 1 seconds + _minutes * 1 minutes + _hours * 1 hours + _days * 1 days + _weeks * 1 weeks;
        if (deadline != 0) {
